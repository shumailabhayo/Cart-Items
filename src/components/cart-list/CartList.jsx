    ...
    <>
      <Drawer open={openCartList} onClose={() => toggleCartLiist(false)}>
        <Box sx={{ width: '380px' }}>
          ...
          {cartItems?.map((item) => {
            return (
              <Box className="row py-2" sx={{ padding: '0px', margin: '0px' }} key={item.id}>
                ...
                <Box className="col-3 d-flex flex-column">
                  <Button onClick={() => dispatch(decreaseQuantity(item))} variant="outlined" size="small" color="error">
                    <RemoveIcon />
                  </Button>
                  <Button onClick={() => dispatch(increaseQuantity(item))} className="my-1" variant="outlined" size="small" color="success">
                    <AddCircleIcon />
                  </Button>
                  <Button onClick={() => dispatch(deleteProdut(item))} variant="outlined" size="small" color="error">
                    <DeleteIcon />
                  </Button>
                </Box>
              </Box>
